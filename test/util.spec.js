...
describe('Component : Util', () => {
  ...
  describe.only('formatMoney', () => {
    const inputs = [undefined,1, 12, 123, 1234, 12345, 123456, 1234567, 12345.67];
    const outputs = ['€ 0.00','€ 1.00', '€ 12.00', '€ 123.00', '€ 1 234.00', '€ 12 345.00', '€ 123 456.00', '€ 1 234 567.00', '€ 12 345.67']
    const outputs2 = ['€ 0.00','€ 1.00', '€ 12.00', '€ 123.00', '€ 1,234.00', '€ 12,345.00', '€ 123,456.00', '€ 1,234,567.00', '€ 12,345.67']
    const outputs3 = ['€ 0,00','€ 1,00', '€ 12,00', '€ 123,00', '€ 1 234,00', '€ 12 345,00', '€ 123 456,00', '€ 1 234 567,00', '€ 12 345,67']
    ...
    it('should format the number into currency with no delimeter', () => {
      ...
      inputs.forEach((num,i)=> {
        ...
        money = Util.toMoneyFormat(num,' ', ',');
        expect(money.startsWith('€'),`2:${i} - Out : ${money}`).to.be.true;
        expect(money,`3:${i} - Out = ${money}`).to.be.eq(outputs3[i]);
      });
    });
  });
